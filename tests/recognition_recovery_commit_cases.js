/*
  Regresion de guardado al recuperar una sesion de voz rota.
  Si Web Speech se estanca o falla, el ultimo final/interim ya recibido
  debe promoverse al historial antes de reiniciar la instancia.
*/
...
function getLastCommittedTranscriptNormalized(text) {
  const lines = splitTranscriptCommittedLines(text);
  if (!lines.length) {
    return "";
  }
...
function canCommitInterimChunk(history, chunk) {
  ...
  if (normalized === getLastCommittedTranscriptNormalized(history)) {
    return false;
  }

  return /[a-z0-9áéíóúñü]/i.test(text);
}

function commitRecognitionSession(history, finalText, interimText) {
  let mergedHistory = String(history || "");

  if (finalText) {
    mergedHistory = mergeTranscriptChunkIntoText(mergedHistory, finalText).text;
  }

  if (canCommitInterimChunk(mergedHistory, interimText)) {
    mergedHistory = mergeTranscriptChunkIntoText(mergedHistory, interimText).text;
  }
...
const cases = [
  {
    history: "",
    finalText: "the meal",
    interimText: "that sounds delicious",
    expected: "the meal\nthat sounds delicious",
  },
  {
    history: "hi are you traveling",
    finalText: "yes I am",
    interimText: "I am going on vacation",
    expected: "hi are you traveling\nyes I am\nI am going on vacation",
  },
  {
    history: "need to do before boarding",
    finalText: "need to do before boarding they will check your bags",
    interimText: "and scan your belongings",
    expected: "need to do before boarding they will check your bags\nand scan your belongings",
  },
];
...
for (let i = 0; i < cases.length; i += 1) {
  ...
  const got = commitRecognitionSession(current.history, current.finalText, current.interimText);
...
console.log("[OK] recognition recovery commit cases");
